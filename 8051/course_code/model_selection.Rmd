...
data.generation<-function(n,p,intercept=0,beta,rho,sig){
  ...
  diag(Sigma)<-rep(1,p) # compound symmetry correlation
...
Check multicollinearity, then fit a ridge, lasso, or elastic net model.
```{r}
...
plot(cv.out)
ridge.mod <- glmnet(x, y, alpha=0, lambda = cv.out$lambda.min)
...
# alpha = 0 does ridge regression, alpha = 1 is lasso.
