...
outlier.check <- function(lmod) {
  ...
  plot(lmod, which = 1)
  plot(lmod, which = 3)
  halfnorm(cooks.distance(lmod), main = "Halfnorm Plot")
  plot(lmod, which = 4)
  par(mfrow = c(1, 1))
...
confmat.regularized <- function(data){
  ...
  ridge.mod <- glmnet(x[train,], y[train], family = "binomial", alpha=0)
  cv.out.r <- cv.glmnet(x[train,], y[train], family = "binomial", alpha=0)  
  ridge.probpred <- drop(predict(ridge.mod, s = cv.out.r$lambda.1se, 
                                 newx = x[test, ], type = "response"))
  ridge.pred <- as.numeric(ridge.probpred > 0.5)
  print.confmat(ridge.pred, y.test, "Ridge")
...
new.compare <- function(comp.df, idx, data, n2, r, 
  ...
  if (abic) {
    comp.df[c(1:2, 10:11), idx] <- cv.abic.compare(data, n2, r)
  } else {
    comp.df[c(1:2, 10:11), idx] <- 0
  }
  comp.df[c(3:6, 12:15), idx] <- cv.regularized.compare(data, n2, r)
  comp.df[c(7:9, 16:18), idx] <- cv.ml.compare(data, n2, r, 
...
cv.regularized.compare <- function(data,n2,r){
  ...
  result <- foreach (i = 1:r, .combine = rbind) %dopar% {
    ...
    # combine the predictions and get accuracy and auc metrics
    preds <- cbind(ridge.pred, lasso.pred, mcp.pred, scad.pred)
    probpreds <- cbind(ridge.probpred, lasso.probpred, mcp.probpred, scad.probpred)
    acc <- apply(preds, 2, function(pred) accuracy_score(pred, y.test))
    auc <- apply(probpreds, 2, function(probpred) auc_score(probpred, y.test))
    c(acc, auc)
    
  }
  
  model.names <- c('ridge', 'lasso', 'mcp', 'scad')
  colnames(result) <- c(sapply(model.names, function(x) paste(x, "acc", sep=".")), 
                        sapply(model.names, function(x) paste(x, "auc", sep=".")))
...
cv.abic.compare<-function(data,n2,r){
  ...
  result <- foreach (i = 1:r, .combine = rbind) %dopar% {
    ...
    probpreds <- cbind(aic.probpred, bic.probpred)
    acc <- apply(preds, 2, function(pred) accuracy_score(pred, y.test))
    auc <- apply(probpreds, 2, function(probpred) auc_score(probpred, y.test))
    c(acc, auc)
  }
...
cv.ml.compare<-function(data,n2,r, xgb.params, xgb.nrounds){
  ...
  result <- foreach (i = 1:r, .combine = rbind) %dopar% {
    ...
    rf.pred <- as.numeric(predict(rf.mod, newdata = data[test, ]))-1
    ...
    svm.pred <- as.numeric(svm.pred)-1
    
    # combine the predictions and get accuracy and auc metrics
    preds <- cbind(xgb.pred, rf.pred, svm.pred)
    probpreds <- cbind(xgb.probpred, rf.probpred, svm.probpred)
    acc <- apply(preds, 2, function(pred) accuracy_score(pred, y.test))
    auc <- apply(probpreds, 2, function(probpred) auc_score(probpred, y.test))
    c(acc, auc)
...
cv.glm.compare <- function(data,n2,r, glm.forms, glm.names){
  ...
  result <- foreach (i = 1:r, .combine = 'rbind', 
    ...
    # prediction by glm models
    glm.preds <- foreach(glm.form=glm.forms, .combine = 'cbind') %do% {
     glm.mod <- glm(glm.form, family=binomial, data[train, ])
     glm.probpred <- predict(glm.mod, newdata = data[test, ], type = "response")
     glm.pred <- as.numeric(glm.probpred > 0.5)
     cbind(glm.pred, glm.probpred)
    }
    
    preds <- glm.preds[, c(TRUE, FALSE)] # odd columns
    probpreds <- glm.preds[, c(FALSE, TRUE)] # even columns
...
# tune the cutoff using repeated cv
# note that logistic regression models are probability-based, so cutoff
  # 0.5 should be the best choice. Pay special attention whenever other
  # cutoff has better performance.
tune.glm.cutoff <-function(data, n2, r, glm.form, 
  ...
  y <- data$y
  n <- nrow(x)
  result <- foreach (cutoff = cutoff.seq, .combine = 'c', 
                     .packages = c('foreach', 'stats')) %dopar% {
  
    foreach(i=1:r, .combine = c) %do% {
      set.seed(i*99) # set seed to ensure reproducibility
      train <- sample(1:n, n-n2)
      test <- (-train)
      y.test <- y[test]
      ...
      # fit a glm model
      glm.mod <- glm(glm.form, family=binomial, data[train, ])
      glm.probpred <- predict(glm.mod, newdata = data[test, ], 
                             type = "response")
      glm.pred <- as.numeric(glm.probpred > cutoff)
  ...
  names(result) <- cutoff.seq
  result
}


# tune cutoff for XGBoost
# the results can be unstable, need to pay special attention, 
  # need to change n2 and r and run multiple times
tune.xgb.cutoff <-function(data, n2, r, xgb.params, xgb.nrounds, 
                           cutoff.seq=seq(0.05, 0.95, by=0.05)){
  ...
  n <- nrow(x)
  result <- foreach (cutoff = cutoff.seq, .combine = 'c', 
                     .packages = c('foreach', 'xgboost')) %dopar% {
    ...
    foreach(i=1:r, .combine = c) %do% {
      ...
      xgb.mod <- xgb.train(params = xgb.params, dtrain, nrounds = xgb.nrounds)
      xgb.probpred <- predict(xgb.mod, X.test)
      xgb.pred <- as.numeric(xgb.probpred > cutoff)
...
select.features.regularized <- function(data, seed = 99) {
  ...
  # use Ridge to get the selected features
  set.seed(seed)
  cv.out <- cv.glmnet(x,y,family="binomial",alpha=0,nfold=10)
  lambdah <- cv.out$lambda.1se
  ridge.mod <- glmnet(x,y,family="binomial",alpha=0,lambda=lambdah) 
  betah <- ridge.mod$beta
  ridge.features <- betah[which(abs(betah) > 0)]
  names(ridge.features) <- colnames(x)[which(abs(betah) > 0)]
  ...
  list(ridge.features = ridge.features, 
       lasso.features = lasso.features, 
       mcp.features = mcp.features, 
       scad.features = scad.features)
}

select.features.abic <- function(data, lower=~1) {
  
  lmod <- glm(y ~ ., family=binomial, data)
  
  # use AIC to get the selected features
  aic.mod <- step(lmod, k = 2, scope = list(upper=~.,lower=lower), 
                  direction = 'both', trace=FALSE)
  
  # use BIC to get the selected features
  bic.mod <- step(lmod, k = log(nrow(data)), 
                  scope = list(upper=~.,lower=lower), 
                  direction = 'both', trace=FALSE)
...
##### ========== XGBoost ============ #####
tune.xgb <- function(X_train, y_train) {
  # use cv to tune parameters for xgboost: both classifier and regressor
...
loo.glm <- function(data, glm.forms, glm.names){
  # use leave-one-out to estimate the accuracy of the avg of glm models
  # data here should be the whole training set
  # assumes the name of the response column is "y"
  ...
  result <- foreach (i = 1:n, .combine = 'rbind', 
    ...
    # prediction by glm models: regsubset, AIC, BIC, LASSO, MCP, SCAD
    preds <- foreach(glm.form=glm.forms, .combine = 'cbind') %do% {
     glm.mod <- glm(glm.form, family=binomial, data[train, ])
     glm.probpred <- predict(glm.mod, newdata = data[test, ], type = "response")
     glm.pred <- as.numeric(glm.probpred > 0.5)
     glm.pred
    ...
    # get accuracy metrics for all methods
    acc <- apply(preds, 2, function(pred) accuracy_score(pred, y.test))
  ...
  colnames(result) <- glm.names
  acc.mean <- apply(result, 2, mean)
  acc.sd <- apply(result, 2, sd)
  
  rbind(acc.mean, acc.sd)
}


loo.avg <- function(data, glm.forms, glm.names, 
  ...
  result <- foreach (i = 1:n, .combine = 'rbind', 
    ...
    ridge.mod <- glmnet(x[train, ],y[train],family="binomial",alpha=0,
                        lambda=ridge.lambda)
    ridge.probpred <- drop(predict(ridge.mod, newx = matrix(x[test, ], nrow = 1), 
                                   type = "response"))
    ridge.pred <- as.numeric(ridge.probpred > 0.5)
    ...
    svm.mod <- svm(formula = as.factor(y) ~ ., data = data[train, ],
                   type = 'C-classification', kernel = 'linear',
                   probability = TRUE)
    svm.pred <- as.numeric(predict(svm.mod, newdata = data[test, ]))-1
    
    # preds <- cbind(glm.preds, glm.preds)
    preds <- cbind(glm.preds, ridge.pred, xgb.pred, rf.pred, svm.pred)
    avg.pred <- as.numeric(rowMeans(preds) > 0.5) # max voting can stabilize prediction
  ...
  colnames(result) <- c(glm.names, 'ridge', 'xgb', 'rf', 'svm', 'avg')
  acc.mean <- apply(result, 2, mean)
  acc.sd <- apply(result, 2, sd)
...
pred.avg <- function(data, newdata, glm.forms, glm.names, 
  ...
  ridge.mod <- glmnet(x,y,family="binomial",alpha=0,
                      lambda=ridge.lambda)
  ridge.probpred <- drop(predict(ridge.mod, newx = x.test, type = "response"))
  ridge.pred <- as.numeric(ridge.probpred > 0.5)
  ...
  # prediction by RF
  rf.mod <- randomForest(as.factor(y) ~ ., data = data)
  rf.pred <- as.numeric(predict(rf.mod, newdata = newdata))-1
  
  # prediction by SVM
  svm.mod <- svm(formula = as.factor(y) ~ ., data = data,
                 type = 'C-classification', kernel = 'linear',
                 probability = TRUE)
  svm.pred <- as.numeric(predict(svm.mod, newdata = newdata))-1
  
  preds <- cbind(glm.preds, ridge.pred, xgb.pred, rf.pred, svm.pred)
  avg.pred <- as.numeric(rowMeans(preds) > 0.5) # max voting
  preds <- cbind(preds, avg.pred)
  
  for (i in 2:ncol(preds)) {
    print.confmat(preds[, i], preds[, 1])
  }
