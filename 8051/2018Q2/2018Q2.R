...
cv.regularized.compare <- function(data,n2,r){
  ...
  c(ridge.err = mean(err.r), ridge.sd = sd(err.r),
...
cv.abic.compare<-function(data,n2,r){
  ...
  result <- foreach (i = 1:r, .combine = rbind) %dopar% {
    ...
    c(err.a.both, err.b.both, err.a.back, err.b.back)
  }
  ...
  c(aic.err.both = mean(err.a.both), aic.sd.both = sd(err.a.both),
    bic.err.both = mean(err.b.both), bic.sd.both = sd(err.b.both), 
    aic.err.back = mean(err.a.back), aic.sd.back = sd(err.a.back),
    bic.err.back = mean(err.b.back), bic.sd.back = sd(err.b.back))		
}
...
cv.ml.compare<-function(data,n2,r, xgb.params, xgb.nrounds){
  ...
  c(xgb.err = mean(err.x), xgb.sd = sd(err.x),
    rf.err = mean(err.rf), rf.sd = sd(err.rf))		
}

# compare with linear models
cv.lmod.compare<-function(data,n2,r, lm.form){
  ...
  c(lm.err = mean(err.lm), lm.sd = sd(err.lm))		
}
...
select.features.regularized <- function(data, seed = 99) {
  ...
  lasso.features <- colnames(x)[which(abs(betah) > 0)]
  ...
  mcp.features <- which(abs(coeff.mcp)>0)
  ...
  scad.features <- which(abs(coeff.scad)>0)
...
loo.avg <- function(data, lm.form, bic.form, lasso.form, 
                    mcp.form){
  ...
  result <- foreach (i = 1:n, .combine = rbind) %dopar% {
    ...
    # prediction by BIC
    bic.mod <- lm(bic.form, data[train, ])
    bic.pred <- predict(bic.mod, newdata = data[test, ])
    mse.bic <- mean((bic.pred - y.test)^2)
    mae.bic <- mean(abs(bic.pred - y.test))
    
    # prediction by lasso
    lasso.mod <- lm(lasso.form, data[train, ])
    lasso.pred <- predict(lasso.mod, newdata = data[test, ])
    mse.lasso <- mean((lasso.pred - y.test)^2)
    mae.lasso <- mean(abs(lasso.pred - y.test))
    
    # prediction by mcp
    mcp.mod <- lm(mcp.form, data[train, ])
    mcp.pred <- predict(mcp.mod, newdata = data[test, ])
...
pred.avg <- function(data, newdata, lm.form, bic.form, lasso.form, 
                    mcp.form){
  # use leave-one-out to estimate the accuracy of the avg of lm and lasso
  # data here should be the whole training set
  # assumes the name of the response column is "y"
  ...
  # prediction by BIC
  bic.mod <- lm(bic.form, data)
  bic.pred <- predict(bic.mod, newdata = newdata)
  
  # prediction by lasso
  lasso.mod <- lm(lasso.form, data)
  lasso.pred <- predict(lasso.mod, newdata = newdata)
  
  # prediction by mcp
  mcp.mod <- lm(mcp.form, data)
  mcp.pred <- predict(mcp.mod, newdata = newdata)

  avg.pred <- 0.3*lm.pred + 0.4*lasso.pred + 0.15*bic.pred + 0.15*mcp.pred
  
  par(mfrow = c(2, 2))
  plot(lm.pred, bic.pred, xlab = "LM", ylab = "BIC")
  abline(0, 1)
  plot(lm.pred, lasso.pred, xlab = "LM", ylab = "LASSO")
  abline(0, 1)
  plot(lm.pred, mcp.pred, xlab = "LM", ylab = "MCP")
  abline(0, 1)
  plot(lm.pred, avg.pred, xlab = "LM", ylab = "AVG")
  abline(0, 1)
  par(mfrow = c(1, 1))
